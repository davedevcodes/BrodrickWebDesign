...
interface ProjectFormProps {
  ...
  defaultValues?: {
    title?: string; slug?: string; client_name?: string; category?: string;
    description?: string; results?: string; tech_stack?: string[];
    live_url?: string; case_study_url?: string; featured_image?: string;
    ...
    <form action={formAction} className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div>
          ...
          <input name="title" required value={title}
            ...
            placeholder="Lumière Studio" className={inputClass} />
        </div>
        <div>
          <label className={labelClass}>Slug *</label>
          <input name="slug" required value={slug}
            onChange={(e) => { setSlug(e.target.value); setSlugEdited(true); }}
            placeholder="lumiere-studio" className={inputClass} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div>
          ...
          <input name="client_name" defaultValue={defaultValues?.client_name || ""}
            placeholder="Client or company name" className={inputClass} />
        </div>
        <div>
      ...
      <div>
        ...
        <textarea name="description" rows={4} defaultValue={defaultValues?.description || ""}
          placeholder="What this project is about..." className={`${inputClass} resize-none`} />
      </div>

      <div>
        <label className={labelClass}>Results</label>
        <textarea name="results" rows={3} defaultValue={defaultValues?.results || ""}
          placeholder="What outcomes were achieved..." className={`${inputClass} resize-none`} />
      </div>

      <div>
        <label className={labelClass}>Tech Stack <span className="text-text-secondary/60">(comma-separated)</span></label>
        <input name="tech_stack"
          defaultValue={defaultValues?.tech_stack?.join(", ") || ""}
          placeholder="Next.js, Tailwind CSS, Supabase" className={inputClass} />
      </div>

      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        <div>
          <label className={labelClass}>Live URL</label>
          <input name="live_url" type="url" defaultValue={defaultValues?.live_url || ""}
            placeholder="https://example.com" className={inputClass} />
        </div>
        <div>
          <label className={labelClass}>Case Study URL</label>
          <input name="case_study_url" type="url" defaultValue={defaultValues?.case_study_url || ""}
            placeholder="https://example.com/case-study" className={inputClass} />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between rounded-xl border border-border bg-background p-4">
        <div>
          ...
          <p className="text-xs text-text-secondary">Show this project in the homepage preview</p>
        </div>
